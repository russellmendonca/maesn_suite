...
rl2 = buildRL2("/home/russellm/generativemodel_tasks/maml_rl_fullversion/data/local/RL2_testResults/RL2_test_pusher_data.pkl", 100)
...
trpoMasenImprove = plot("/home/russellm/generativemodel_tasks/maml_rl_fullversion/data/local/masenTRPOImprovement-val1-rate0.01paramSpace/","Task",97)
trpoMamlImprove = plot("/home/russellm/maml_rl_baseline_test/data/local/mamlTRPOImprovement-val1-rate0.01/","Task",97)
...
plt.plot(np.arange(0,3), np.mean(masen_STDprior, 0)[:3], label = "MAESN", color = "red")
plt.plot(np.arange(2,100), np.mean(masen_STDprior, 0)[2:],  color = "red", linestyle ="dashed")
plt.plot(np.arange(3,100), np.mean(trpoMasenImprove,0), color = "red")

plt.plot(np.arange(0,3), np.mean(maml_ADA, 0)[:3], label = "MAML", color="green")
plt.plot(np.arange(2,100), np.mean(maml_ADA,0)[2:], color="green", linestyle="dashed")
plt.plot(np.arange(3,100), np.mean(trpoMamlImprove,0), color="green")

plt.plot(np.arange(0,100), rl2, label="rl2")

plt.plot(np.arange(0,100), np.mean(trpo,0), label='TRPO', color="grey")

plt.plot(np.arange(0,100), np.mean(vpg_val,0), label='VPG')

plt.plot(np.arange(0,100), np.mean(LS, 0), label = "LatentSpace")
...
plt.legend(loc=4)
plt.savefig("randSparsePusher-trainedOn100-valSet1-trpoImprovement-100tasks-10itr.png")
