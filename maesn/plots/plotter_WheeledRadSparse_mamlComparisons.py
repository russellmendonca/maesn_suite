...
def plotMeanStd(numItrs, var, label, marker = None):

    return plt.plot(np.arange(0,numItrs), np.mean(var,0), label=label, markEvery = 10, marker = marker)
...
a1, = plotMeanStd(100, masen, "MASEN (Ours)", "d" )
a2, =plotMeanStd(100, maml, "MAML", 's')

#plotMeanStd(100, LS, "LS")
a3, =plotMeanStd(100, mamlBiasBiasADA, "MAML+BiasTransform+onlyBiasAdaptation", "^")
a4, =plotMeanStd(100, mamlBiasFullADA, "MAML+BiasTransform+allParameterAdaptation", "P")
...
plt.title("Ablation Study on Wheeled Locomotion")
