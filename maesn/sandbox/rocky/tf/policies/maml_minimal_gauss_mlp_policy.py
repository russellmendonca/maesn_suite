...
class MAMLGaussianMLPPolicy(StochasticPolicy, Serializable):
    def __init__(
        ...
        if std_network is not None:
            ...
            self._prior_f_dist = tensor_utils.compile_function(
                inputs=[self.input_tensor, self.task_idx, self.zs],
                outputs=[mean_var, log_std_var],
            )
    ...
    def compute_updated_dists(self, samples, samples_latent, plot=None):
        ...
        if 'all_fast_params_tensor' not in dir(self):
            ...
            for i in range(num_tasks):
                ...
                fast_params_tensor = OrderedDict(zip(all_keys, [self.all_params[key] - step_size*tf.convert_to_tensor(gradients[key]) for key in all_keys]))
    ...
    def updated_dist_info_sym(self, task_id, surr_obj, surr_obj_latent, new_obs_var, new_task_idx_var, new_noise_var, params_dict=None, params_dict_latent=None, is_training=True):
        ...
        params_dict_latent = dict(zip(update_param_keys_latent, [old_params_dict_latent[key] - step_size*gradients_latent[key] for key in update_param_keys_latent]))
