...
class BatchMAMLPolopt(RLAlgorithm):
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            if self.load_policy_vals is not None:


              
                load_policy_vals_actual = pickle.load(open(self.load_policy_vals, "rb"))
                pars = self.policy.get_params()


                print(self.reset_step)
                for par in pars:
                    if par.name in load_policy_vals_actual:
                        if 'stepsize' in par.name: 
                            if self.reset_step == 'True':
                                print("SKIPPED " + par.name)
                                continue
                            elif self.reset_step == 'scale':
                                a_op = par.assign(load_policy_vals_actual[par.name]/10.)
                                sess.run(a_op)
                                print(par.name)
                            else:    
