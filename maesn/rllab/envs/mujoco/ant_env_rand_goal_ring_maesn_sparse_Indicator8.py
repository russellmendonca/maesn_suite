...
class AntEnvRandGoalRing(MujocoEnv, Serializable):
    ...
    def step(self, action):
        ...
        if np.linalg.norm(com[:2] - self.goals[self._goal_idx]) < 0.8:
            goal_reward = -np.sum(np.abs(com[:2] - self.goals[self._goal_idx])) + 4.0 # make it happy, not suicidal
            distance_reward = -np.sum(np.abs(com[:2] - self.goals[self._goal_idx]))
        else:
            goal_reward = -np.sum(np.abs(self.goals[self._goal_idx])) + 4.0 # make it happy, not suicidal
            distance_reward = -np.sum(np.abs(self.goals[self._goal_idx]))

        dense_goal_reward = -np.sum(np.abs(com[:2] - self.goals[self._goal_idx])) + 4.0 
        ...
        return Step(ob, float(reward), done, distance_reward = distance_reward, dense_reward = dense_reward)
